import React from 'react'
import './Customerreview.scss'

export default function Customerreview() {
  return (
    <>
        <div className='container-fluid' id="customer-c-f">
            <div className='container' id="customer-c">
                <div className='row'> 
                    <div className='col-lg-6 col-md-6 col-sm-12'>
                        <div className='col'>
                            <h3>Client Testimonials</h3>
                        </div>
                    </div>
                    <div className='col-lg-6 col-md-6 col-sm-12'>
                        <div className='col p-4' id="cust-item-wrap">
                            <div className='row' id="cust-item">
                                <div className='col-3'>
                                    <img src="https://images.pexels.com/photos/18042477/pexels-photo-18042477/free-photo-of-a-bearded-man-in-a-denim-jacket-standing-outside.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" alt=""></img>
                                </div>
                                <div className='col-8'>
                                    <p>Boost your product and service's credibility by adding testimonials from your clients. People love recommendations so feedback from others who've tried it is invaluable.</p>
                                    <h6>- Wendy Salinas, Rolk Inc</h6>
                                </div>
                            </div>
                        </div>  
                        <div className='col p-4' id="cust-item-wrap">
                            <div className='row' id="cust-item">
                                <div className='col-3'>
                                    <img src="https://images.pexels.com/photos/18042477/pexels-photo-18042477/free-photo-of-a-bearded-man-in-a-denim-jacket-standing-outside.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" alt=""></img>
                                </div>
                                <div className='col-8'>
                                    <p>Boost your product and service's credibility by adding testimonials from your clients. People love recommendations so feedback from others who've tried it is invaluable.</p>
                                    <h6>- Wendy Salinas, Rolk Inc</h6>
                                </div>
                            </div>
                        </div>  
                        <div className='col p-4' id="cust-item-wrap">
                            <div className='row' id="cust-item">
                                <div className='col-3'>
                                    <img src="https://images.pexels.com/photos/18042477/pexels-photo-18042477/free-photo-of-a-bearded-man-in-a-denim-jacket-standing-outside.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" alt=""></img>
                                </div>
                                <div className='col-8'>
                                    <p>Boost your product and service's credibility by adding testimonials from your clients. People love recommendations so feedback from others who've tried it is invaluable.</p>
                                    <h6>- Wendy Salinas, Rolk Inc</h6>
                                </div>
                            </div>
                        </div>         
                    </div>
               
                </div>
            </div>
        </div>
    </>
  )
}
